...
## Фаза 2.5: Interpret project context (если аргумент передан)

**Триггер:** пользователь вызвал `/setup-agent-harness <свободный текст>`. Если аргумент пустой — фаза пропускается, `SPECIFIC_RULES` = "" в фазе 7.

### 2.5.1 Сохранить raw input

`PROJECT_CONTEXT = <весь аргумент как есть>`. Ключ понадобится в фазе 7.4 для `harness-config.json` — при будущих `/harness-update` можно будет перерендерить SPECIFIC_RULES из того же контекста без потери намерения.

### 2.5.2 Интерпретировать текст → набор правил

Пройтись по типовым осям и решить, какие из них триггернуты словами пользователя. Каждая ось = категория, дающая 2-5 конкретных правил.

| Ось (сигнальные слова) | Категория правил | Что добавить в SPECIFIC_RULES |
|------------------------|-------------------|-------------------------------|
| «MVP», «прототип», «скорость», «быстро», «черновик», «lean» | **Speed-first** | Skip pre-flight и mid-retro для <30 LOC changes. Task-файл минимум: 3 строки (context + AC + план). PR max 200 строк. docs — плейсхолдеры, полная документация не блокирует merge |
| «прод», «продакшн», «uptime», «reliability», «SLA», «HA» | **Reliability-first** | Pre-flight обязателен для любого изменения ≥5 LOC. Health-check + graceful degradation обсуждаются в task-файле. Rollback plan — секция в каждом task-файле |
| «платежи», «финансы», «PCI», «money», «biling», «charging», «HIPAA», «PII» | **Compliance-first** | Audit-log в `docs/audit/YYYY-MM-DD-*.md` для любого касания payment-code. External code-reviewer subagent обязателен на task-done. Secret-scan strict: любой match в `git diff` — блокер, не warning |
| «research», «experiment», «prototype», «throwaway», «hack», «playground» | **Exploration-first** | AC в task-файле — опционально. Breaking API changes не требуют миграции. docs-sync не обязателен. Focus: скорость экспериментов |
| «high-uptime», «zero-downtime», «feature flag», «canary», «staged» | **Rollout-safe** | Feature flag mandatory для любой user-visible feature. `docs/flags.md` — каталог активных флагов. Staged deployment: canary → 10% → 50% → 100%. Rollback trigger в task-файле |
| «команда», «team», «shared codebase», «>1 разработчик» | **Team-mode** | Каждый task-файл до commit'а — review одним subagent (`code-reviewer`). Комментарии по-русски для юзеров, английский — для кода. Naming conventions — фиксировать в `docs/conventions.md` |
| «solo», «one-person», «моно», «personal» | **Solo-mode** | Убрать формальность внешних ревью. Комментарии произвольные. Task-файлы могут быть краткими |
| «legacy», «brownfield», «унаследованный», «монолит» | **Legacy-aware** | Не менять существующий код без причины. Каждое structural change — обосновать в task-файле. Тесты добавлять к каждому касанию untested-области |
| «AI/ML», «data science», «notebooks», «tune», «train» | **Data-science-flavor** | Reproducibility: fix random seeds. `docs/experiments/YYYY-MM-DD-*.md` для каждой tune-задачи. Model versioning документируется |

Оси **не взаимно исключающие** — «прод + платежи + команда» = сумма правил всех трёх. Ось выбирается, если хотя бы одно сигнальное слово встречается (case-insensitive, substring match).

Если ни одна ось не триггернулась — сгенерировать один общий пункт: «Контекст пользователя: `<PROJECT_CONTEXT>` — учитывать при принятии решений о scope, детализации, приоритетах.»

### 2.5.3 Сгенерировать SPECIFIC_RULES блок
...
**Исходный контекст:** _"<PROJECT_CONTEXT>"_

Из него агент вывел следующие правила специфичные для этого проекта:
...
Если оси не триггернулись — блок состоит из одного `**Контекст:**` и одного generic-правила.

### 2.5.4 Показать пользователю, дать возможность править

Одной репликой:

> Из контекста «<PROJECT_CONTEXT>» я вывел правила: <краткий 1-строчный сводный список категорий>. Полный блок готов к вставке в CLAUDE.md. Согласен, или поправить/удалить какие-то? (по умолчанию через 10 секунд — иду дальше как есть)

Ждать 3-10 секунд. Если пользователь молчит — считать согласованным. Если корректирует — применить корректировку, снова показать, дождаться `ok`.

Полный сгенерированный markdown-блок — сохранить как значение переменной `SPECIFIC_RULES` для фазы 7.
...
Скопировать 10 файлов из `references/skills/` в `.claude/skills/` целевого проекта:
...
- `consolidate.md` — периодический sweep закрытых задач на кросс-задачные паттерны в E-категории self-review

**Утилиты:**
- `harness-update.md` — sync `.claude/skills/` проекта с последними templates из harness'а (v1 — только skill'ы)
...
**Идемпотентный докат:** если из 10 файлов часть уже есть, а часть отсутствует — доложить недостающие. Это применимо к старым проектам, где harness был поставлен до появления skill'ов само-улучшения.
...
- `{{SPECIFIC_RULES}}` — сгенерировано на фазе 2.5 из аргумента skill'а. Если аргумент не был передан — пустая строка (backward compat). Если был — markdown-блок с секцией «Проектные особенности (интерпретация PROJECT_CONTEXT)».

### 7.3 Дополнить контекстными правилами

Если анализ фазы 3 выявил специфику (например, обязательный запуск миграций, специфичная система CI, критические инварианты) — добавить компактным списком в раздел «Проектные особенности». **Не путать** с блоком из фазы 2.5 (SPECIFIC_RULES) — фаза 2.5 работает с намерением пользователя, фаза 7.3 — с фактами из codebase. Если оба присутствуют — обе секции живут рядом.
...
```json
{
  ...
  "PROJECT_CONTEXT": "жёсткий MVP, скорость важнее детализации",
  "SPECIFIC_RULES": "## Проектные особенности (интерпретация PROJECT_CONTEXT)\n..."
}
```

`PROJECT_CONTEXT` — сырой аргумент пользователя из фазы 2.5 (или `""` если аргумент не был). Хранится отдельно от `SPECIFIC_RULES`, чтобы `/harness-update` мог перегенерировать SPECIFIC_RULES из того же контекста, не теряя намерение при эволюции правил интерпретации.

Если проект уже имел `.claude/harness-config.json` (повторный вызов `/setup-agent-harness`) — merge: сохранить существующие ключи, добавить новые если появились. Особый случай `PROJECT_CONTEXT`: если пользователь **не передал** новый аргумент, старое значение сохраняется; если передал — старое замещается новым.
...
- Проверить, всех ли **10 skills** хватает. Если недостаёт — предложить доложить недостающие (без перезаписи существующих).
- Спросить пользователя: «Harness уже установлен. Что обновить?»
  - (a) Только skills (перезаписать 10 файлов из свежих шаблонов) — эквивалентно `/harness-update` + CLAUDE.md-sync (T-036, пока руками).
