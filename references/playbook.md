...
Скопировать 9 файлов из `references/skills/` в `.claude/skills/` целевого проекта:
...
- Проверить, всех ли **9 skills** хватает. Если недостаёт — предложить доложить недостающие (без перезаписи существующих).
- Спросить пользователя: «Harness уже установлен. Что обновить?»
  - (a) Только skills (перезаписать 9 файлов из свежих шаблонов) — эквивалентно `/harness-update` + CLAUDE.md-sync (T-036, пока руками).
