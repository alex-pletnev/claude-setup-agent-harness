...
Скопировать 7 файлов из `references/skills/` в `.claude/skills/` целевого проекта:
...
**Идемпотентный докат:** если из 7 файлов часть уже есть, а часть отсутствует — доложить недостающие. Это применимо к старым проектам, где harness был поставлен до появления skill'ов само-улучшения.
...
- Проверить, всех ли **7 skills** хватает. Если недостаёт — предложить доложить недостающие (без перезаписи существующих).
- Спросить пользователя: «Harness уже установлен. Что обновить?»
  - (a) Только skills (перезаписать 7 файлов из свежих шаблонов).
