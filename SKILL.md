...
6. **Skills install** — 10 skills копируются в `.claude/skills/` (4 таск-трекерных + 5 само-улучшения + `harness-update` для будущих sync-ов).
...
- `.claude/skills/` содержит ровно 10 файлов (task-add, task-done, task-sync, docs-sync, wheel-check, mid-retro, self-review, pre-flight, harness-update, consolidate).
